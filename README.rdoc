...
== jfeust Fork
This fork adds a "single_invitor" option.  In the normal devise_invitable gem, if an invite is not yet accepted and a different user invites the same email address, that user becomes the "invited_by" value instead of the original.  In cases where invites are like currency, you want the first person to invite someone to always remain the invitor.

CURRENTLY UNDER DEVELOPMENT. DON'T USE YET
